...
/**
 * Performs an NTLM negotiate request against the server.
 *
 * @param {String} the type1 NTLM token.
 * @param {Control} controls (optional) either a Control or [Control].
 * @param {Function} callback of the form f(err, res).
 * @throws {TypeError} on invalid input.
 */
Client.prototype.negotiate = function negotiate(token,
                                                controls,
                                                callback) {
    if (typeof (controls) === 'function') {
        callback = controls;
    ...
    var req = new BindRequest({
        authentication: 'GSS-SPNEGO',
        credentials: token,
        controls: controls
    });

    return this._send(req, [errors.LDAP_SUCCESS,errors.LDAP_SASL_BIND_IN_PROGRESS], null, callback);
};
